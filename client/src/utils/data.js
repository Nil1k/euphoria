...
export const userPinsQuery = (userId, type) => {
	const subQuery = type === 'saved' ? `'${userId}' in save[].userId ]` : `userId == '${userId}']`
	const query =`*[_type == 'pin' && ${subQuery} | order(_createdAt desc){
		image{ asset -> {url} },
		_id,
		destination,
		postedBy -> { _id, userName, image },
